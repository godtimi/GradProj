// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        v5.28.3
// source: yymall-srvs/userop_srv/proto/userfav.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type UserFavRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        int32                  `protobuf:"varint,1,opt,name=userId,proto3" json:"userId,omitempty"`
	GoodsId       int32                  `protobuf:"varint,2,opt,name=goodsId,proto3" json:"goodsId,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UserFavRequest) Reset() {
	*x = UserFavRequest{}
	mi := &file_yymall_srvs_userop_srv_proto_userfav_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserFavRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserFavRequest) ProtoMessage() {}

func (x *UserFavRequest) ProtoReflect() protoreflect.Message {
	mi := &file_yymall_srvs_userop_srv_proto_userfav_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserFavRequest.ProtoReflect.Descriptor instead.
func (*UserFavRequest) Descriptor() ([]byte, []int) {
	return file_yymall_srvs_userop_srv_proto_userfav_proto_rawDescGZIP(), []int{0}
}

func (x *UserFavRequest) GetUserId() int32 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *UserFavRequest) GetGoodsId() int32 {
	if x != nil {
		return x.GoodsId
	}
	return 0
}

type UserFavResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        int32                  `protobuf:"varint,1,opt,name=userId,proto3" json:"userId,omitempty"`
	GoodsId       int32                  `protobuf:"varint,2,opt,name=goodsId,proto3" json:"goodsId,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UserFavResponse) Reset() {
	*x = UserFavResponse{}
	mi := &file_yymall_srvs_userop_srv_proto_userfav_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserFavResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserFavResponse) ProtoMessage() {}

func (x *UserFavResponse) ProtoReflect() protoreflect.Message {
	mi := &file_yymall_srvs_userop_srv_proto_userfav_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserFavResponse.ProtoReflect.Descriptor instead.
func (*UserFavResponse) Descriptor() ([]byte, []int) {
	return file_yymall_srvs_userop_srv_proto_userfav_proto_rawDescGZIP(), []int{1}
}

func (x *UserFavResponse) GetUserId() int32 {
	if x != nil {
		return x.UserId
	}
	return 0
}

func (x *UserFavResponse) GetGoodsId() int32 {
	if x != nil {
		return x.GoodsId
	}
	return 0
}

type UserFavListResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Total         int32                  `protobuf:"varint,1,opt,name=total,proto3" json:"total,omitempty"`
	Data          []*UserFavResponse     `protobuf:"bytes,2,rep,name=data,proto3" json:"data,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UserFavListResponse) Reset() {
	*x = UserFavListResponse{}
	mi := &file_yymall_srvs_userop_srv_proto_userfav_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserFavListResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserFavListResponse) ProtoMessage() {}

func (x *UserFavListResponse) ProtoReflect() protoreflect.Message {
	mi := &file_yymall_srvs_userop_srv_proto_userfav_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserFavListResponse.ProtoReflect.Descriptor instead.
func (*UserFavListResponse) Descriptor() ([]byte, []int) {
	return file_yymall_srvs_userop_srv_proto_userfav_proto_rawDescGZIP(), []int{2}
}

func (x *UserFavListResponse) GetTotal() int32 {
	if x != nil {
		return x.Total
	}
	return 0
}

func (x *UserFavListResponse) GetData() []*UserFavResponse {
	if x != nil {
		return x.Data
	}
	return nil
}

var File_yymall_srvs_userop_srv_proto_userfav_proto protoreflect.FileDescriptor

const file_yymall_srvs_userop_srv_proto_userfav_proto_rawDesc = "" +
	"\n" +
	"*yymall-srvs/userop_srv/proto/userfav.proto\x1a\x1bgoogle/protobuf/empty.proto\"B\n" +
	"\x0eUserFavRequest\x12\x16\n" +
	"\x06userId\x18\x01 \x01(\x05R\x06userId\x12\x18\n" +
	"\agoodsId\x18\x02 \x01(\x05R\agoodsId\"C\n" +
	"\x0fUserFavResponse\x12\x16\n" +
	"\x06userId\x18\x01 \x01(\x05R\x06userId\x12\x18\n" +
	"\agoodsId\x18\x02 \x01(\x05R\agoodsId\"Q\n" +
	"\x13UserFavListResponse\x12\x14\n" +
	"\x05total\x18\x01 \x01(\x05R\x05total\x12$\n" +
	"\x04data\x18\x02 \x03(\v2\x10.UserFavResponseR\x04data2\xec\x01\n" +
	"\aUserFav\x123\n" +
	"\n" +
	"GetFavList\x12\x0f.UserFavRequest\x1a\x14.UserFavListResponse\x125\n" +
	"\n" +
	"AddUserFav\x12\x0f.UserFavRequest\x1a\x16.google.protobuf.Empty\x128\n" +
	"\rDeleteUserFav\x12\x0f.UserFavRequest\x1a\x16.google.protobuf.Empty\x12;\n" +
	"\x10GetUserFavDetail\x12\x0f.UserFavRequest\x1a\x16.google.protobuf.EmptyB\tZ\a.;protob\x06proto3"

var (
	file_yymall_srvs_userop_srv_proto_userfav_proto_rawDescOnce sync.Once
	file_yymall_srvs_userop_srv_proto_userfav_proto_rawDescData []byte
)

func file_yymall_srvs_userop_srv_proto_userfav_proto_rawDescGZIP() []byte {
	file_yymall_srvs_userop_srv_proto_userfav_proto_rawDescOnce.Do(func() {
		file_yymall_srvs_userop_srv_proto_userfav_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_yymall_srvs_userop_srv_proto_userfav_proto_rawDesc), len(file_yymall_srvs_userop_srv_proto_userfav_proto_rawDesc)))
	})
	return file_yymall_srvs_userop_srv_proto_userfav_proto_rawDescData
}

var file_yymall_srvs_userop_srv_proto_userfav_proto_msgTypes = make([]protoimpl.MessageInfo, 3)
var file_yymall_srvs_userop_srv_proto_userfav_proto_goTypes = []any{
	(*UserFavRequest)(nil),      // 0: UserFavRequest
	(*UserFavResponse)(nil),     // 1: UserFavResponse
	(*UserFavListResponse)(nil), // 2: UserFavListResponse
	(*emptypb.Empty)(nil),       // 3: google.protobuf.Empty
}
var file_yymall_srvs_userop_srv_proto_userfav_proto_depIdxs = []int32{
	1, // 0: UserFavListResponse.data:type_name -> UserFavResponse
	0, // 1: UserFav.GetFavList:input_type -> UserFavRequest
	0, // 2: UserFav.AddUserFav:input_type -> UserFavRequest
	0, // 3: UserFav.DeleteUserFav:input_type -> UserFavRequest
	0, // 4: UserFav.GetUserFavDetail:input_type -> UserFavRequest
	2, // 5: UserFav.GetFavList:output_type -> UserFavListResponse
	3, // 6: UserFav.AddUserFav:output_type -> google.protobuf.Empty
	3, // 7: UserFav.DeleteUserFav:output_type -> google.protobuf.Empty
	3, // 8: UserFav.GetUserFavDetail:output_type -> google.protobuf.Empty
	5, // [5:9] is the sub-list for method output_type
	1, // [1:5] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_yymall_srvs_userop_srv_proto_userfav_proto_init() }
func file_yymall_srvs_userop_srv_proto_userfav_proto_init() {
	if File_yymall_srvs_userop_srv_proto_userfav_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_yymall_srvs_userop_srv_proto_userfav_proto_rawDesc), len(file_yymall_srvs_userop_srv_proto_userfav_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   3,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_yymall_srvs_userop_srv_proto_userfav_proto_goTypes,
		DependencyIndexes: file_yymall_srvs_userop_srv_proto_userfav_proto_depIdxs,
		MessageInfos:      file_yymall_srvs_userop_srv_proto_userfav_proto_msgTypes,
	}.Build()
	File_yymall_srvs_userop_srv_proto_userfav_proto = out.File
	file_yymall_srvs_userop_srv_proto_userfav_proto_goTypes = nil
	file_yymall_srvs_userop_srv_proto_userfav_proto_depIdxs = nil
}
